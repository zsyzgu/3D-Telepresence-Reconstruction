...
extern "C" void cudaIntegrate(int cameras, int& triSize, Vertex* vertex, UINT16** depth, float* depth_device, RGBQUAD** color, Transformation* toWorldTrans, Transformation* depthToColorTrans, Intrinsics* depthIntrinsics, Intrinsics* colorIntrinsics);
...
void TsdfVolume::integrate(byte* result, int cameras, UINT16** depth, float* depth_device, RGBQUAD** color, Transformation* depthTrans, Transformation* colorTrans, Intrinsics* depthIntrinsics, Intrinsics* colorIntrinsics)
{
	...
	cudaIntegrate(cameras, *((int*)result), vertex, depth, depth_device, color, depthTrans, colorTrans, depthIntrinsics, colorIntrinsics);
}
